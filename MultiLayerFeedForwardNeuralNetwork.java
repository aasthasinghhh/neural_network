...
abstract public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
	...
	public void backprop(Example example, double alpha) {
		...
		double[][] deltaValues = new double [this.layers.length][];
		NeuronUnit[] outputs = this.getOutputUnits();
		//
		deltaValues[this.layers.length-1] = new double [outputs.length];
		...
		for(int j=0; j<outputs.length; j++){
			...
			deltaValues[this.layers.length-1][j]=tmp;
		}

		for(int l = this.layers.length-2;l >= 1;l--) {
			deltaValues[l] = new double [this.layers[l].length];
			for(int i = 0; i < this.layers[l].length; i++){
				...
				for(int j = 0; j < deltaValues[l+1].length; j++) {
					List<Connection> i_jConnections = this.layers[l][i].outgoingConnections;
					tempSum += deltaValues[l+1][j]*i_jConnections.get(j).weight;
				}
				deltaValues[l][i] = gPrime*tempSum;
			}
		}

		for(int k = 0; k < this.layers.length-1; k++) {
			for(int n = 0; n<this.layers[k].length; n++) {
				List<Connection> mn_Out = this.layers[k][n].outgoingConnections;

				for(int w = 0; w < mn_Out.size();w++) {
					mn_Out.get(w).weight = mn_Out.get(w).weight+alpha * this.layers[k][n].getOutput() * deltaValues[k+1][w];
				}
			}
		}
	}
	/**  Connection conn = mn_Out.get(w);
            double gradient = this.layers[m][n].getOutput() * deltaArrays[m + 1][w];
            double deltaWeight = (alpha * gradient) + (mu * conn.prevDeltaWeight) - (lambda * conn.weight);
            conn.weight += deltaWeight;
            conn.prevDeltaWeight = deltaWeight;  */
	/**
